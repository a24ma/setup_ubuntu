...
on_failed() { echo "failed on $1"; rm -f out; exit 1; }
require() { if [[ -z "$(eval "echo \$${1:?}")" ]]; then on_failed "undefined:$1"; fi }

edit_with_backup() {
    require top_id
    usage="usage: $0 <edit_path> <setup_id> <backup_id> <comment_str> (REQUIRE: 'out' is required to be added to edit_path.)"
    edit_path="${1:?$usage}"
    setup_id="${2:?$usage}"
    backup_id="${3:?$usage}"
    comment_str="${4:?$usage}"
